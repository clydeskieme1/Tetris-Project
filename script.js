...
function createPiece() {
  ...
  const shape = shapes[typeId]; // Get the shape
  return {
    shape,
...
function drawPiece(piece, ctx = context) {
  ...
  piece.shape.forEach((row, dy) => {
    row.forEach((value, dx) => {
      if (value) {
        ctx.fillRect((piece.x + dx) * grid, (piece.y + dy) * grid, grid, grid);
...
function drawNextPiece(piece) {
  ...
  const offsetX = Math.floor(
    (nextCanvas.width - piece.shape[0].length * grid) / 2
  );
  // Center the piece horizontally
  const offsetY = Math.floor(
    (nextCanvas.height - piece.shape.length * grid) / 2
  );
...
function updateHighScore(score) {
  ...
  document.getElementById(
    "high-score"
  ).textContent = `High Score: ${localStorage.getItem("highScore")}`;
